...
(defvar keywiz-right-phrases '("Excellent!" "Yes." "Indeed." "You're right."
			       "t." "Spot-on." "Correct." "Yep."))
(defvar keywiz-wrong-phrases '("Nope." "Wrong." "Nah." "nil." "No."
			       "Incorrect." "(beep)" "Huh?"))
...
(defvar keywiz-score-file (expand-file-name "keywiz-scores"
					    keywiz-temp-dir)
  "File for holding high scores.")

(defvar keywiz-time-limit 120)
...
to pause keywiz and enter a recursive edit in the *scratch* buffer."
  (interactive "P")
  (let ((score 0) (first t) commands start-time timer doc key input quit)
    (random t)
	...
	(when (and (commandp sym)
		   (not (memq sym '(self-insert-command digit-argument))))
	...
	(progn
	  ...
	  (erase-buffer)
	  (keywiz-insert-with-face 'keywiz-heading-face "Welcome to keywiz\n")
...
will return to keywiz.  This allows you to try out a command.")
	  (while (and commands
		      (not quit)
		      (< (- (keywiz-float-time) start-time) keywiz-time-limit))
	    (setq commands (remove (setq key (keywiz-random commands)) commands))
	    ...
	    (cond
	     ((member input (cadr key))
	      (keywiz-insert-with-face 'keywiz-right-face
				       (keywiz-random keywiz-right-phrases)
				       "  The answer is: "
				       (mapconcat 'key-description (cadr key) ", "))
	      (incf score))
	     ((setq quit (and (member input '([?q] [q])) (y-or-n-p "Do you want to quit? "))))
	     ((member input '([?r] [r]))
	      ;; 'r' means recursive muck-around with the timer stopped
	      (keywiz-insert-with-face 'keywiz-wrong-face "Time-out time-out! Question skipped.")
	      (cancel-timer timer)
		...
		(incf start-time (- (keywiz-float-time) timeout-start))))
	     (t
	      (keywiz-insert-with-face 'keywiz-wrong-face
