...
\\\\]" (regexp-quote comment-start) "\\(.+\\)$") end t)) (modes . align-open-comment-modes)) (c-macro-definition (regexp . "^\\s-*#\\s-*define\\s-+\\S-+\\(\\s-+\\)") (modes . align-c++-modes)) (c-variable-declaration (regexp . "[*&0-9A-Za-z_]>?[&*]*\\(\\s-+[*&]*\\)[A-Za-z_][0-9A-Za-z:_]*\\s-*\\(\\()\\|=[^=
 ...
 '(find-function-source-path (append load-path (mapcar (lambda (p) (concat as-emacs-dir "/" p)) (list "fun" "major-modes/monkey-2" "major-modes/mmm" "major-modes/tdtd" "major-modes/pcl-cvs" "major-modes" "minor-modes" "utils"))))
