...
;; Author: Oliver Seidel <os10000@myrealbox.com>
;; Created: 2 Aug 1997
;; Modified: 25 May 2005 Todd Neal <tolchz@tolchz.net>  Ver: 1.1
...
;;
;;   In calendar-mode  
;;          T   insert a new entry on the highlighted date
;;
;;  Configuration
;;
...
(defcustom todo-print-header "TODO"
  "Header for PostScript in printed output"
  :type 'string
  :group 'todo)
...
(defun todo-delete-item ()
  ...
  (if (> (count-lines (point-min) (point-max)) 0)
      (let* ((inhibit-read-only t)
	     (todo-entry (todo-item-string-start))
...
between each category."
  ...
  (let ((todo-print-buffer-name todo-tmp-buffer-name)
    ...
    ;;Remove the todo-prefix text from the buffer
    (while (re-search-forward (regexp-quote (concat todo-prefix " ")) nil t)
...
Number of entries for each category is given by `todo-print-priorities'."
  ...
  (save-window-excursion
	...
	(set-buffer todo-tmp-buffer-name)
	(font-lock-fontify-buffer)
	(let ((ps-left-header (list (concat "(" todo-print-header ")"))))
...
(defun todo-mail ()
"Email todo summary to `user-mail-address' 
Number of entries for each category is given by `todo-print-priorities'."
  (interactive)
  (save-window-excursion
    (save-excursion
      (save-restriction
	(todo-top-priorities todo-print-priorities nil)
	(set-buffer todo-tmp-buffer-name)
	(progn
	  (message "Sending mail")
	  (compose-mail user-mail-address "TODO")
	  (insert-buffer todo-tmp-buffer-name)
...
(define-generic-mode 'todo-mode
  ...
  (list 
   ;; todo-prefix
   (list (regexp-quote todo-prefix) 0 font-lock-comment-face)
   ;; date
   (list "[0-9]+-[0-9]+-[0-9]+[ \t]*\\([0-9]+:[0-9]+ \\)*\\(.*\\):" 0 font-lock-type-face)
   )
...
(define-generic-mode 'todo-disp-mode
  ;; comment character
  nil
  ;; keywords
  nil
  ...
  "Syntax highlighting mode only of todo-print buffer")
...
(defun todo-calendar-setkeys ()
  (define-key calendar-mode-map "T"
    (lambda ()
      (interactive)
      (progn
	(save-excursion
	  (let* ((cursor-date (calendar-cursor-to-date t))
		 ...
		 (new-item (concat todo-prefix " "
				   (read-from-minibuffer
				    "New TODO entry: "
				    (if t
					(format "%d-%d-%d %s : " year month day todo-initials))))))
	    (todo-show)
	    (let* ((inhibit-read-only t)
		   (current-category (first (first (todo-category-alist))))
		   (category (completing-read (concat "Category [" current-category "]: ")
					      (todo-category-alist) nil nil nil
					      nil current-category)))
	      (todo-add-item-non-interactively new-item category)
	      )
					; Delete the buffer after inserting the entry ?
					;(kill-buffer (current-buffer))
	    ))))))


(add-hook 'calendar-load-hook 'todo-calendar-setkeys)
